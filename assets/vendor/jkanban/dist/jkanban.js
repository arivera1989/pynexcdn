...
(function () {
  this.jKanban = function () {
    ...
    this.removeElement = function (el) {
      console.log(el);
      if (typeof el === 'string')
        el = self.element.querySelector('[data-idTarea="' + el + '"]')
      if (el !== null) {
        //fallback for IE
...
/*
    function __buildItemCard(item) {
      ...
      function __buildItemCard(item) {
        // Crear el contenido HTML con los datos de la tarea
        var taskName = 'name' in item ? item.name : 'Sin título';
        var label = 'label' in item ? item.label : 'Sin etiqueta';
        var creationDate = 'creationDate' in item ? item.creationDate : 'Fecha no disponible';
        var formattedCreationDate = creationDate !== 'Fecha límite no disponible' ;
        var dueDate = 'dueDate' in item ? item.dueDate : 'Fecha límite no disponible';
        var formattedDueDate = dueDate !== 'Fecha límite no disponible' ;
        var comment = 'comment' in item ? item.comment : 'Sin comentarios';
    
        // Construir el contenido HTML
        var result = `
            <span id="prioridad" class="badge bg-label-${label} mb-2">Prioridad</span><br>
            <strong id="nombre">${taskName}</strong><br>
            <span id="vencimiento">Vence: ${dueDate}</span><br>
        `;
    
        // Si las opciones de manejo de ítems están habilitadas
        if (self.options.itemHandleOptions.enabled) {
            if (!self.options.itemHandleOptions.customHandler) {
                var customCssHandler = self.options.itemHandleOptions.customCssHandler || 'drag_handler';
                var customCssIconHandler = self.options.itemHandleOptions.customCssIconHandler || customCssHandler + '_icon';
    
                result = `
                    <div class='item_handle ${customCssHandler}'>
                        <i class='item_handle ${customCssIconHandler}'></i>
                    </div>
                    <div>
                        ${result}
                    </div>`;
                ...
                result = `
                    <div>
                        ${self.options.itemHandleOptions.customHandler.replace(/%([^%]+)%/g, function (match, key) {
